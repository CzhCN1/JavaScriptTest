...
var mine = (function(){
	 ...
	 */
	var addHandler= function (element, eventType, handler) {
	 ...
	 * @param {[type]} handler   [事件回调函数]
	 */
	var removeHandler= function(element, eventType, handler){
		if (element.addEventListener) {				//DOM2级事件处理(不支持IE)
			//false为事件冒泡
	 ...
	 */
	var getByClass= function  (oParent, className) {
	 ...
	 */
	var move= function  (obj,attrs,fn) {
	 ...
	 */
	var drag= function (obj, oWrap) {
		//为拖拽对象添加鼠标按下事件
		addHandler(obj, 'mousedown', function (event){
			//规则检测
			try{
				//检测该对象是否为绝对定位
				...
				if(oWrap){
					...
					while(target.tagName != 'BODY'){
						if(target === oWrap){
							break;
						}else if(getStyle(target,'position')  != 'static'){
					...
					//判断范围对象是否已定位
					//若未定位则不能作为偏移参照基准
					if(getStyle(oWrap,'position') == 'static'){
